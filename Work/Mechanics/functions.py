...
# При l_progonu=0 (по замовчанню) коефцієнт k_L буде розраховано по спрощенному варіанту
def find_P_m(l_progonu=0):
    ...
    C_dc = g_tu * find_alfa() * find_k_L(l_progonu)
    ...
    C_c = find_C_h() * C_rel * C_dir  # C_rel, C_dir, C_c - продубльовані в find_P_m(), find_Q_m
...
def find_Q_m(l_progonu=0):
    ...
    Q_m = Q_om * find_miu_g() * find_k_g() * C_c * find_k_L(l_progonu) * math.sin(math.radians(fi)) ** 2
    return Q_m
...
def find_gamma_4(l_progonu=0):
    gamma_4 = find_P_m(l_progonu) / cables[cable]['S']
    return gamma_4


# Від дії вітру на провід вкритий ожеледдю - МПа/м (Q_m - Н/м, S - мм**2)
def find_gamma_5(l_progonu=0):
    gamma_5 = find_Q_m(l_progonu) / cables[cable]['S']
    return gamma_5


# Від власної ваги проводу і дії вітру на провід без ожеледі - МПа/м:
def find_gamma_6(l_progonu=0):
    gamma_6 = math.sqrt(find_gamma_1() ** 2 + find_gamma_4(l_progonu) ** 2)
    return gamma_6


# Від дії вітру на провід, вкритий ожеледдю, власної ваги, ваги ожеледі під час дії вітру на провід, вкритий ожеледдю
#  МПа/м
def find_gamma_7(l_progonu=0):
    gamma_7 = math.sqrt((find_gamma_1() + 0.9 * find_gamma_2()) ** 2 + find_gamma_5(l_progonu) ** 2)
    return gamma_7
...
def find_gamma_ozh_nb(l_progonu=0):
    gamma_ozh_nb = max(find_gamma_3(), find_gamma_7(l_progonu))
    return gamma_ozh_nb
...
def find_l_1k():
    if ((6 * (betta * (sigma_sr - sigma_nb) + alfa_temp * (t_ser - t_min))) /
        ...
        print('ПОМИЛКА ПРИ РОЗРАХУНКУ ПЕРШОГО КРИТИЧНОГО ПРОГОНУ')
        return False
    else:
...
def find_l_2k(l_progonu=0):
    if 6*alfa_temp*(t_ozhel - t_min)/((find_gamma_ozh_nb(l_progonu)/find_gamma_1())**2 - 1) < 0:
        ...
        return (2 * sigma_nb / find_gamma_1() * math.sqrt(6 * alfa_temp * (t_ozhel - t_min) /
                ((find_gamma_ozh_nb(l_progonu) / find_gamma_1()) ** 2 - 1)))
    
    
# Функція для розрахунку третього критичного прогону (формула 8.5):
def find_l_3k(l_progonu=0):
    if (6*(betta*(sigma_nb - sigma_sr) + alfa_temp*(t_ozhel - t_ser)) /
            ((find_gamma_ozh_nb(l_progonu)/find_gamma_1())**2 - (sigma_nb/sigma_sr)**2)) < 0:
        print('ПОМИЛКА ПРИ РОЗРАХУНКУ ТРЕТЬОГО КРИТИЧНОГО ПРОГОНУ')
        return False
    else:
        return 2 * sigma_nb / find_gamma_1() * math.sqrt(
            6 * (betta * (sigma_nb - sigma_sr) + alfa_temp * (t_ozhel - t_ser)) /
            ((find_gamma_ozh_nb(l_progonu)/find_gamma_1()) ** 2 - (sigma_nb / sigma_sr) ** 2))
